...
def test_create_chunks_searchable_process_not_supported(document_collection, requests_mock):
    requests_mock.get(
        f"{API_BASE}/buildInfo",
        headers={"Content-Type": "application/json"},
        json={
...
def test_create_and_run_chunks_searchable_process_not_supported(document_collection, requests_mock):
    requests_mock.get(
        f"{API_BASE}/buildInfo",
        headers={"Content-Type": "application/json"},
        json={
    ...
    with pytest.raises(OperationNotSupported):
        document_collection.create_and_run_process(
            process_name="test-process", pipeline="test-pipeline", send_chunks_to_search=True
        )
